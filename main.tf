...
locals {
  ...
  runners_defaults = {
    ...
    ami                       = var.runners_executor == "docker+machine" ? data.aws_ami.docker_machine[0].id : ""
    security_group_name       = var.runners_executor == "docker+machine" ? aws_security_group.docker_machine[0].name : ""
...
resource "aws_iam_role_policy_attachment" "config_bucket" {
  # TODO not sure if this is correct. Have to check why this policy is needed
