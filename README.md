...
If you want to use existing key pair set `ssh_key_pair` variable.
...
| gitlab\_runner\_ssh\_cidr\_blocks | List of CIDR blocks to allow SSH Access from to the gitlab runner instance. | list(string) | `<list>` | no |
