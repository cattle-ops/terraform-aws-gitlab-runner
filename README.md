...
A few [examples](examples) is provided, execute the following steps to run the sample. Ensure your AWS and Terraform environment is set up correctly. All commands below are supposed to be run inside the directory of the examples.
...
*This step is not needed anymore*, configure you runner via `gitlab_runner_registration_config`. Configure GitLab via the step below is only needed when you choose to crate the token manually and set thea `runners_token` variable. 
