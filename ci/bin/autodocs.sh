...
pandoc -v &>/dev/null || {
    echo >&2 "ERROR: Pandoc not installed"
    exit 1
}
terraform-docs --version &>/dev/null || {
    echo >&2 "ERROR: terraform-docs not installed"
    exit 1
}
...
for i in "${arr[@]}"; do
    ...
    if [[ -d "$docs_dir" ]]; then
        ...
        if ! test -f $docs_dir/README.md; then
            echo "ERROR: _docs dir found with no README.md"
            exit 1
        fi
        ...
        terraform-docs $1 $2 markdown $i >README.md
    fi
done
