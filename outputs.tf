...
output "runner_cache_bucket_arn" {
  ...
  value       = element(concat([for s3_cache_bucket in module.cache : s3_cache_bucket.arn], [""]), 0)
}

output "runner_cache_bucket_name" {
  description = "Name of the S3 for the build cache."
  value       = element(concat([for s3_cache_bucket in module.cache : s3_cache_bucket.bucket], [""]), 0)
}
...
output "runner_eip" {
  ...
  value       = element(concat([for gitlab_runner in aws_eip.gitlab_runner : gitlab_runner.public_ip], [""]), 0)
}
