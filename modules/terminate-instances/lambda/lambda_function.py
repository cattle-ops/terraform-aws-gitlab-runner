...
def ec2_list(client, **args):  # noqa: C901

    print(
        json.dumps(
            {
                "Level": "info",
                "Message": "Searching for children of"
                f" GitLab runner instance {args['parent']}",
            }
        )
    )
    ...
    ec2_instances = client.describe_instances(
        Filters=[
            ...
            {"Name": "tag:gitlab-runner-parent-id", "Values": ["*"]},
        ]
    ).get("Reservations")
            ...
            for tag in instance["Tags"]:
                ...
                if tag["Key"] == "gitlab-runner-parent-id":
                    if tag["Value"] == args["parent"]:
                            ...
                            json.dumps(
                                {
                                    ...
                                    " will be terminated"
                                    " because its parent is terminating.",
                                }
                            )
                        )
                        ...
                        try:
                            # Handle instances without a parent that are still running.
                            _other_child = client.describe_instances(
                                InstanceIds=[tag["Value"]]
                            )
                            # The parent is still in the inventory as 'terminated'
                            if len(_other_child["Reservations"]) > 0:
                                if (
                                    _other_child["Reservations"][0]["Instances"][0][
                                        "State"
                                    ]["Name"]
                                    == "terminated"
                                ):
                            ...
                            if "InvalidInstanceID.NotFound" in str(e):
                                ...
                                # Handle any other exception and skip this instance.
                                print(
                                    json.dumps(
                                        {"Level": "exception", "Exception": str(e)}
                                    )
                                )
                                continue
                            ...
                            json.dumps(
                                {
                                    ...
                                    "Name": _name,
                                    "LaunchTime": str(instance["LaunchTime"]),
                                    "Message": f"{instance['InstanceId']} appears to"
                                    " be orphaned. Parent runner"
...
def handler(event, context):
    ...
    if len(_terminate_list) > 0:
        ...
        try:
            ...
            print(json.dumps({"Level": "exception", "Exception": str(e)}))
            raise Exception(
                f"Encountered exception when terminating instances: {str(e)}"
            )
    else:
        print(json.dumps({"Level": "info", "Message": "No instances to terminate."}))
