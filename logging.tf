resource "aws_iam_role_policy" "instance" {
  count  = var.agent_cloudwatch_enable && var.agent_create_runner_iam_role_profile ? 1 : 0
...
locals {
  logging_user_data = templatefile("${path.module}/template/logging.tftpl",
    {
      log_group_name = var.agent_cloudwatch_log_group_name != null ? var.agent_cloudwatch_log_group_name : var.environment
  })
...
resource "aws_cloudwatch_log_group" "environment" {
  count = var.agent_cloudwatch_enable ? 1 : 0
  name  = var.agent_cloudwatch_log_group_name != null ? var.agent_cloudwatch_log_group_name : var.environment
  # ignores a false positive: retention_in_days not set
  # kics-scan ignore-line
  retention_in_days = var.agent_cloudwatch_retention_days
  tags              = local.tags
