...
resource "aws_security_group" "runner" {
  ...
  vpc_id      = var.vpc_id
  description = var.runner_security_group_description
...
resource "aws_security_group_rule" "runner_ping_group" {
  count = length(var.runner_ping_allow_from_security_groups) > 0 && var.runner_ping_enable ? length(var.runner_ping_allow_from_security_groups) : 0
  ...
  source_security_group_id = element(var.runner_ping_allow_from_security_groups, count.index)
  security_group_id        = aws_security_group.runner.id

  description = format(
    "Allow ICMP traffic from %s to gitlab-runner agent instances in group %s",
    element(var.runner_ping_allow_from_security_groups, count.index),
...
locals {
  ...
  security_groups_ping = var.runner_ping_enable && length(var.runner_ping_allow_from_security_groups) > 0 ? concat(var.runner_ping_allow_from_security_groups, [aws_security_group.runner.id]) : []
}
...
resource "aws_security_group_rule" "docker_machine_ping_runner" {
  count = var.runner_worker_type == "docker+machine" ? length(local.security_groups_ping) : 0
...
resource "aws_security_group_rule" "docker_machine_ping_self" {
  count = (var.runner_worker_type == "docker+machine" && var.runner_ping_enable) ? 1 : 0
